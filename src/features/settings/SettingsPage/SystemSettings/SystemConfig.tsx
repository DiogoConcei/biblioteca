...
import { SortAsc, ArrowUp, ArrowDown, Save, Globe } from 'lucide-react';
...
export default function SystemConfig() {
  ...
  useEffect(() => {
    if (selectedId != null) {
      ...
      if (
        selectedSerie &&
        (selectedSerie.literatureForm === 'Manga' ||
          selectedSerie.literatureForm === 'Quadrinho')
      ) {
  ...
  return (
    <section className={styles.card}>
      ...
      <section className={styles.reorderSection}>
        ...
        <p>
          Selecione uma série no grid ou no menu abaixo para realizar ações de
          reordenação ou consulta de metadados.
        </p>
        ...
        <div className={styles.seriesSelectorRow}>
          ...
          >
            ...
            {reorderableSeries.map((serie) => (
              <option key={serie.id} value={serie.dataPath}>
                {serie.name} (
                {serie.literatureForm === 'Manga' ? 'Mangá' : 'Quadrinho'})
              </option>
            ))}
          </select>
          ...
          {selectedSeriePath && (
            ...
            >
              <Globe size={16} />{' '}
              {isFetchingMetadata ? 'Buscando...' : 'Buscar Dados Online'}
            </button>
          )}
        </div>
        ...
        {chapters.length > 0 && (
          <div className={styles.reorderContainer}>
            ...
            <div className={styles.mainActions}>
              ...
              >
                <Save size={16} />{' '}
                {isSavingOrder ? 'Salvando...' : 'Salvar Nova Ordem'}
              </button>
            </div>
          </div>
        )}

...
export default class ImageOperations extends FileSystem {
  ...
  public async createComicById(serieName: string, chapter_id: number): Promise<string> {
    try {
      ...
      console.error(`Erro ao processar o capítulo com ID "${chapter_id}" em "${serieName}": ${error}`);
      throw error;
    }
  }
  ...
  public async createComic(serieName: string, quantity: number): Promise<string> {
    try {
      ...
      const serieNames = path.basename(serieData.archives_path);
      const seriePath = await this.fileManager.findJsonFile(serieNames);
      const lastDownload = await this.dataManager.foundLastDownload(serieNames);
...
//     await MangaOperations.createComic("SPY×FAMILY", 7)
//   } catch (error) {

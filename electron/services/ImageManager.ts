...
export default class ImageManager extends FileSystem {
  ...
  public async normalizeChapter(dirPath: string): Promise<void> {
    try {
      ...
      for (const entry of entries.filter(e => e.isDirectory())) {
        await this.storageManager.fixComicDir(path.join(entry.parentPath, entry.name), dirPath);
      }

      const fixEntries = (await fse.readdir(dirPath, { withFileTypes: true })).map(entry =>
  ...
  public async normalizeImage(imagePath: string): Promise<string> {
    let imageInstance: sharp.Sharp | null = null;

    try {
      const imageFilter = /\.(jpg|jpeg|png|gif|bmp|webp|tiff)$/i;
      if (!imageFilter.test(imagePath)) return '';
      ...
      const destPath = path.join(this.showcaseImages, `${sanitizedFileName}.webp`);

      if (ext === '.webp' && path.basename(normalizedPath) === `${sanitizedFileName}.webp`) {
  ...
  public async clearChapter(dirChapter: string): Promise<void> {
    try {
      ...
      const imageFiles = pathDirents
        .filter(dirent => dirent.isFile() && /\.(jpeg|png|tiff|jpg)$/i.test(dirent.name))
  ...
  public async decodeBase64ToImage(serieName: string, codePath: string): Promise<string> {
    try {

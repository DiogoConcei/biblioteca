...
export default function chaptersHandlers(ipcMain: IpcMain) {
  ...
  ipcMain.handle('chapter:get-single', async (_event, serieName: string, chapter_id: number) => {
    try {
      ...
      switch (LiteratureForm) {
        case 'Mangas':
          return (chapterData = await mangaManager.getChapter(dataPath, chapter_id));
        case 'Comics':
          return (chapterData = await comicManager.getComic(dataPath, chapter_id));
        default:
          break;
      }

      return { success: true, chapterData: chapterData };
    } catch (e) {
      console.error(`Erro ao recuperar o capítulo: ${e}`);
      return { success: false, error: String(e) };
    }
  });
  ...
  ipcMain.handle(
    ...
    async (_event, serieName: string, chapter_id: number, page_number: number) => {
      try {
        const dataPath = await fileManager.getDataPath(serieName);
  ...
  ipcMain.handle('chapter:acess-last-read', async (_event, dataPath: string) => {
    try {
      const serieData = await storageManager.readSerieData(dataPath);
      ...
      if (!lastChapter)
        throw new Error(`Último capítulo não encontrado para a série ${serieData.name}`);

      if (!lastChapter.isDownload) {
        switch (literatureForm) {
          case 'Mangas':
            await mangaManager.createEditionById(serieData.dataPath, lastChapter.id);
            break;
          case 'Comics':
            await comicManager.createEditionById(serieData.dataPath, lastChapterId);
            break;
          default:
            break;
        }
  ...
  ipcMain.handle('get-next-chapter', async (_event, serieName: string, chapter_id: number) => {
    try {
      ...
      const nextChapter = serieData.chapters?.find(chapter => chapter.id === chapter_id + 1);
  ...
  ipcMain.handle('get-prev-chapter', async (_event, serieName: string, chapter_id: number) => {
    try {
      ...
      const serieData = await storageManager.readSerieData(dataPath);
      const prevChapter = serieData.chapters?.find(chapter => chapter.id === chapter_id - 1);

...
export default function downloadHandlers(ipcMain: IpcMain) {
  ...
  ipcMain.handle('multiple-download', async (_event, dataPath: string, quantity: number) => {
    try {
  ...
  ipcMain.handle('single-download', async (_event, dataPath: string, chapter_id: number) => {
    ...
    if (chapter.isDownload) {
      await storageManager.deleteSerieChapter(serieData, chapter, literatureForm);
      return true;
    }
  ...
  ipcMain.handle('donwload-in-reading', async (_event, serieName: string, chapter_id: number) => {
  ...
  ipcMain.handle('check-download', async (_event, serieName: string, chapter_id: number) => {
    const dataPath = await fileManager.getDataPath(serieName);
    if (!dataPath) {
      throw new Error(`Data path not found for serie: ${serieName}`);
    }
    ...
    try {
      const alreadyDownloaded = await validationManager.checkDownload(serieData, chapter_id);
